...
void RTScene::init(void) {
    ...
    model["table piece"]->material = material["silver"];
    ...
    model["teapot1"] = new RTModel;
    model["teapot1"]->geometry = geometry["cube"];
    model["teapot1"]->material = material["silver"];
    model["teapot2"] = new RTModel;
    model["teapot2"]->geometry = geometry["cube"];
    model["teapot2"]->material = material["wood"];
    ...
    node["table top"]->modeltransforms.push_back(translate(vec3(0.0f, -0.5f, 0.0f)) * scale(vec3(20.0f, 0.2f, 10.0f)));
    node["table top"]->childnodes.push_back(node["teapot1"]);
    node["table top"]->childtransforms.push_back(translate(vec3(-0.8f, 0.0f, 0.0f)));
    node["table top"]->childnodes.push_back(node["teapot2"]);
    node["table top"]->childtransforms.push_back(translate(vec3(0.8f, 0.0f, 0.0f)) * rotate(-120.0f * float(M_PI) / 180.0f, vec3(0.0f, 1.0f, 0.0f)));
